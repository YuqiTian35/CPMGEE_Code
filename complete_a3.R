...
latent_correlation_matrix <- matrix(0.3, nrow=cluster_size, ncol=cluster_size)
diag(latent_correlation_matrix) <- 1

data <- simulated_cont_data$simdata %>% mutate(y = qchisq(pnorm(y/2), 5))
